...
const priceSell = 10000000

const minBalance = 45000000
    ...
    bot.on('message', async (message) => {
        ...
        if (messageText.includes('[$] Ваш баланс:')) {
            ...
            for (let i = firstInventorySlot; i <= lastInventorySlot; i++) {
                if (bot.inventory.slots[i] && bot.inventory.slots[i].name === 'netherite_chestplate') count++
            }
    ...
    try {
        ...
        if (!bot.ahFull) {
            ...
            for (let i = 0; i < lastInventorySlot; i++) {
                ...
                if (!slot || slot.name !== 'netherite_chestplate') continue;
            ...
            for (let i = 0; i < 9; i++) {
                ...
                if (bot.inventory.slots[slotIndex]?.name === 'netherite_chestplate') {
                ...
                for (let j = 0; j < 27; j++) {
                    ...
                    if (bot.inventory.slots[j]?.name === 'netherite_chestplate') {
    ...
    for (let i = firstAHSlot; i <= lastAHSlot; i++) {
        ...
        try {
            ...
            for (let sellSlot = firstSellSlot; sellSlot <= lastInventorySlot; sellSlot++) {
                ...
                if (item && item?.name === 'netherite_chestplate') countItems++
            }
            let bestPrice = 0
