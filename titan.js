...
const maxPrice = 4700000
const priceSell = 5000000
...
const ahCommand = '/ah search сфера титана 3';
            ...
            for (let sellSlot = firstSellSlot; sellSlot <= lastInventorySlot; sellSlot++) {
                ...
                if (!item) {
                    ...
                    for (let invSlot = firstInventorySlot; invSlot <= lastInventorySlot; invSlot++) {
                        ...
                        if (!invItem || invItem?.name !== 'player_head') continue;
                    ...
                    items[sellSlot - firstSellSlot] = item?.name === 'player_head';
                }
            }
...
async function getBestAHSlot(bot) {
    ...
    for (let i = firstAHSlot; i <= lastAHSlot; i++) {
        ...
        if (slotData.name !== 'player_head') continue;
