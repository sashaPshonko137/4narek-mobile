...
const ahCommand = '/ah search bottle o e';
            ...
            for (let sellSlot = firstSellSlot; sellSlot <= lastInventorySlot; sellSlot++) {
                ...
                if (!item) {
                    ...
                    for (let invSlot = firstInventorySlot; invSlot <= lastInventorySlot; invSlot++) {
                        ...
                        if (!invItem || invItem?.name !== 'experience_bottle') continue;
                    ...
                    items[sellSlot - firstSellSlot] = item?.name === 'experience_bottle';
                }
            }
...
async function getBestAHSlot(bot) {
    ...
    for (let i = firstAHSlot; i <= lastAHSlot; i++) {
        ...
        if (slotData.name !== 'experience_bottle' || slotData.count != 64) continue;
