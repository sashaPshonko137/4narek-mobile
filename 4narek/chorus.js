...
const ahCommand = '/ah search chorus fruit';
    ...
    bot.on('windowOpen', async () => {
        switch (bot.menu) {
            ...
            case buy:
                ...
                await safeClick(bot, Math.floor(Math.random() * 3), 0)
        ...
        try {
            ...
            for (let sellSlot = firstSellSlot; sellSlot <= lastInventorySlot; sellSlot++) {
                ...
                if (!item) {
                    ...
                    items.push(item.name === 'chorus_fruit');
                }
            }
...
async function getBestAHSlot(bot) {
    ...
    for (let i = firstAHSlot; i <= lastAHSlot; i++) {
        ...
        if (slotData.name !== 'chorus_fruit' || slotData.count != 64) continue;
